...
namespace DotNetWikiBot
{
	...
	public class PageList
	{
        ...
        public int CountFileRevisions(string pageTitle)
        {
            if (string.IsNullOrEmpty(pageTitle))
                throw new ArgumentNullException("pageTitle");
            Bot.LogEvent(
                Bot.Msg("Counting file revisons of \"{0}\" page..."), pageTitle);
            string res = site.site + site.indexPath + "index.php?title=" +
            ...
            int count = 0;
            int idx = 0;
            while ((idx = text.IndexOf("Thumbnail for version as of ", idx)) >= 0)
            {
                idx++;
                count++;
            }
